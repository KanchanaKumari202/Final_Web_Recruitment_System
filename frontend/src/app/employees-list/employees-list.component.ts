...
export class EmployeesListComponent implements OnInit {
 ...
 this.restApi.getEmployees().subscribe((data) => {
   console.log(data)
      this.employees = data;
          // to add ratings and status
  //   for (const emp of this.employees) {
  //    this.inde = this.inde + 1;
  //    if (this.inde === 5) {
  //      this.inde = 0;
  //    }
  //    emp.status = this.stat[this.inde];
  //    emp.rates = this.rates[this.inde];
  ...
  //   let  employeeObject 
  //   let dateObject
  //  this.updateUserStatus(employee).then(
  //    res=>{
  //       if(employee.rate > "3"){
  //         this.restApi.sendEmails(employee.email).subscribe(data => {
  //           this.sending=true;
  //         });
  //         dateObject = {
